...
namespace App\Http\Livewire\Admin\Charity\Projects;
    ...
    public function render()
    {
        ...
        if ($this->category_search  != '') {
            $query = $query->whereHas('categories', function($q) {
                $q->where('category_id', 'like', '%' . $this->category_search . '%');
            });
            $this->resetPage();
        }
        ...
        if ($this->search_created_from != '' && $this->search_created_to != '') {
            $fromDate = \Carbon\Carbon::createFromFormat('Y-m-d', $this->search_created_from)->startOfDay();
            $toDate = \Carbon\Carbon::createFromFormat('Y-m-d', $this->search_created_to)->endOfDay();
            $query = $query->whereBetween('created_at', [$fromDate, $toDate]);
            $this->resetPage();
        } elseif ($this->search_created_from != '') {
            $fromDate = \Carbon\Carbon::createFromFormat('Y-m-d', $this->search_created_from)->startOfDay();
            $query = $query->where('created_at', '>=', $fromDate);
            $this->resetPage();
        } elseif ($this->search_created_to != '') {
            $toDate = \Carbon\Carbon::createFromFormat('Y-m-d', $this->search_created_to)->endOfDay();
            $query = $query->where('created_at', '<=', $toDate);
            $this->resetPage();
        }

        // Specific creation date filtering (if not using range)
        if ($this->search_creation_at != '' && $this->search_created_from == '' && $this->search_created_to == '') {
            $creationDate = \Carbon\Carbon::createFromFormat('Y-m-d', $this->search_creation_at);
            $query = $query->whereDate('created_at', '=', $creationDate);
            $this->resetPage();
        }
        ...
        return view('livewire.admin.charity.projects.index', compact('items', 'links', 'categories'));
    }
}
